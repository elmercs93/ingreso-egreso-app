...
export class IngresoEgresoService {
  ...
  initIngresoEgresoListener() {
    this.ingresoEgresoListerSubcription = this.store.select('auth')
      ...
      .subscribe(auth => this.ingresoIngresoItems(auth.user.uid));
  }


  private ingresoIngresoItems(uid: string) {


    this.ingresoEgresoItemsSubcription = this.afDB.collection(`${uid}/ingresos-egresos/items`).snapshotChanges()
      .pipe(
        map(docData => {
